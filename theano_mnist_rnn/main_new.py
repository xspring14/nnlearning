...
def train():
    ...
    model.build(x, y, learning_rate)

    print "loading data..."
    batch_size = 64
    imgs_dir = "/home/xu/data/"
    train_imgs_list = "/home/xu/data/mnist/train/train.txt"
    test_imgs_list = "/home/xu/data/mnist/train/val.txt"
    ...
    for epoch in range(20):
        ...
        for i in range(n_train_steps):
            ...
            x_batches, y_batches = train_prefetcher.next_batch()
            x_batches = np.transpose(x_batches, (1, 0, 2))
            train_cost = model.sgd(x_batches, y_batches, lr)
            print("{}. {}/{} step's loss is {}".format(epoch+1, i+1, n_train_steps, train_cost))

        for i in range(n_test_steps):

            x_batches, y_batches = test_prefetcher.next_batch()
            x_batches = np.transpose(x_batches, (1, 0, 2))
            test_acc = model.predict(x_batches, y_batches)
